...
bscope * mklocalscope(Buckets *buckets) {
    bscope *answer = allocInBuckets(buckets, sizeof(bscope), bk_alignof(bscope));
    answer->scope = local;
    return answer;
}
...
rst * mkapply (Buckets* buckets, rst *fn, int nargs, ...) {
    rstapply* answer = allocInBuckets(buckets, sizeof(rstapply) + sizeof (rst *) * nargs, bk_alignof(rstapply));
...
rst * mkbindto (Buckets* buckets, rst *n, bscope *scope, char *name) {
    rstbind* answer = allocInBuckets(buckets, sizeof(rstbind), bk_alignof(rstbind));
...
rst * mkget (Buckets* buckets, bscope *scope, char *name) {
    rstget* answer = allocInBuckets(buckets, sizeof(rstget), bk_alignof(rstget));
...
rst * mkgetoverload (Buckets* buckets, bscope *scope, char *name, int nargs) {
    rstgetoverload* answer = allocInBuckets(buckets, sizeof(rstgetoverload), bk_alignof(rstgetoverload));
...
rst * mkret (Buckets* buckets, rst *n, bscope *scope) {
    rstret* answer = allocInBuckets(buckets, sizeof(rstret), bk_alignof(rstret));
    answer->base.op = rst_get;
    answer->n = n;
    answer->scope = scope;
    return (rst*) answer;
...
rst * mkseq (Buckets* buckets, int nnodes, ...) {
    va_list args;
    rstseq* answer = allocInBuckets(buckets, sizeof(rstseq) + sizeof(rst *) * nnodes, bk_alignof(rstseq));
...
rst * mkstalloc (Buckets* buckets, int nvars, ...) {
    ...
    rststalloc* answer = allocInBuckets(buckets, sizeof(rststalloc), bk_alignof(rststalloc));
    if (nvars > 0) {
        answer->nvars = nvars;
//        answer->nodes = (rst **) allocInBuckets(buckets, sizeof(rst *) * nvars, bk_alignof(rst *));
        va_start(args, nvars);
...
rst * mklitint (Buckets* buckets, long x) {
    rstlit* answer = allocInBuckets(buckets, sizeof(rstlit), bk_alignof(rstlit));
