...
//            PP(info, "tm_union_for_tlid_or_create - #2");
            ...
            _update_type_summary(tm, btypeid, uniid, 0, hasT);
...
pub btypeid_t * tm_union_tl(BK_TM *tm, btypeid_t btypeid) {
    ...
    btsummary *sum;  TM_TLID_T tlid;  btypeid_t * tl;

    if (!btypeid) return 0; // (btypeid_t *) _err_invalid_btype_B_NAT(0, __FILE__, FN_NAME, __LINE__);
    if (btypeid >= tm->next_btypeId) return 0; // _err_selfOutOfRange(NULL, __FILE__, FN_NAME, __LINE__, btypeid);
