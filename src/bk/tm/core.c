...
pvt inline TM_SLID_TLID slidtupidFromXxxid(hi_struct(TM_DETAILID_BY_SLIDTUPIDHASH) *hi, TM_DETAILID_T detailid) {
    return (TM_SLID_TLID) {.slid = hi->tm->slid_by_strid[detailid], .tlid = hi->tm->tlid_by_strid[detailid]};
}

pvt bool inline slidtupidHashableFound(hi_struct(TM_DETAILID_BY_SLIDTUPIDHASH) *hi, TM_DETAILID_T token, TM_SLID_TLID hashable) {
...
// HI_IMPL(name, token_t, hashable_t, __hash_fn, __found_fn, __hashable_from_token_fn)
HI_IMPL(TM_DETAILID_BY_SLIDTUPIDHASH, TM_DETAILID_T, TM_SLID_TLID, slidtupid_hash, slidtupidHashableFound, slidtupidFromXxxid)
