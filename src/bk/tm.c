...
pub btypeid_t tm_inter(BK_TM *tm, btypeid_t self, btypeid_t *typelist) {
    ...
    for (i = 1; i <= (i32) typelist[0]; i++) {
        if (!(TM_FIRST_VALID_BTYPEID <= typelist[i] && typelist[i] < tm->next_btypeId)) return B_NAT;
...
pvt btypeid_t _inter_for_emplaced_tl(BK_TM *tm, btypeid_t self) {
    u32 idx;  i32 i, outcome;  TM_TLID_T tlid;  TM_DETAILID_T intid;  btypeid_t *p1;  bool hasT;
...
pub btypeid_t tm_new(BK_TM *tm, btypeid_t self, bool isrecursive, btypeid_t orthspcid, bool isexp, btypeid_t implicitid) {
    ...
    tm->btsummary_by_btypeid[self] |=
        (isrecursive ? TM_IS_RECURSIVE_MASK : 0) |
        (isexp ? TM_IS_EXPLICIT_MASK : 0) |
        (orthspcid ? TM_IN_ATMSPC_MASK : 0);
...
pub btypeid_t * tm_inter_tl(BK_TM *tm, btypeid_t self) {
    // answer a symlist ptr to the given intersection's field names or 0 for error
    btsummary *sum;
