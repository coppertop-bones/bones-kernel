...
//// ---------------------------------------------------------------------------------------------------------------------
//// exclusiveAtom: (name:str, exclusionCat:int) -> PyBType + PyException
//
//pvt PyObject * PyTM_exclusiveAtom(PyTM *self, PyObject **args, Py_ssize_t nargs) {
//    // answer a new atom with the given exclusion
...
//    char *name = (char *) PyUnicode_AsUTF8(args[0]);
//    btypesize_t sz = 0;
//    btypeid_t btypeid = tm_exclatom(self->tm, name, excl, sz, 0);
//    if (btypeid)
...
pvt PyObject * PyTM_atom(PyTM *self, PyObject **args, Py_ssize_t nargs) {
    // answer a new atom with the given name, or an exception if already taken
    if (nargs != 1) return jErrWrongNumberOfArgs(FN_NAME, 1, nargs);
    if (!PyUnicode_Check(args[0]) || (PyUnicode_KIND(args[0]) != PyUnicode_1BYTE_KIND)) return PyErr_Format(PyExc_TypeError, "name must be utf8");
    char *name = (char *) PyUnicode_AsUTF8(args[0]);
    btypeid_t btypeid = tm_atom(self->tm, 0, 0, name);
    if (btypeid)
        return newPyBTypeRef(btypeid);
    else
...
//    },
//    {"exclusiveAtom",    (PyCFunction) PyTM_exclusiveAtom, METH_FASTCALL,
//        "exclusiveAtom(t)\n\n"
//        "Answers the exclusive atom called 'name', creating it if it doesn't exist, or raises an error if the name is used by another type."
//    },
    ...
    {"atom",                (PyCFunction) PyTM_atom, METH_FASTCALL,
        "atom(name)\n\n"
