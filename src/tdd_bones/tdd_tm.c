...
pvt TPN test_orthogonal_spaces(BK_MM *mm, Buckets *buckets, BK_TP *tp) {
    ...
    t = tm_interv(tm, B_NEW, 2, GBP, tm_atom(tm, B_NEW, "fred"));
    check(t != B_NAT, "%s @ %i: GBP & USD != B_NAT", __FILE__, __LINE__);
...
pvt TPN test_minus(BK_MM *mm, Buckets *buckets, BK_TP *tp) {
    ...
    // test simple orthogonality
    t3 = tm_atom(tm, B_NEW, "GBP");        // mem is a built-in type? don't think so
    t2 = tm_atom(tm, B_NEW, "ccy");
    t1 = tm_atom(tm, B_NEW, "f64");
    t4 = tm_atom(tm, B_NEW, "u32");
...
pvt TPN test_construction_extended(BK_MM *mm, Buckets *buckets, BK_TP *tp) {
    ...
    FX = tm_name_as(tm, tm_reserve(tm, B_NEW, ccyfx, 0, 0), "FX");
    ...
    fx = tm_name_as(tm, tm_reserve(tm, B_NEW, FX, 0, 0), "fx");
    fx = tm_interv(tm, fx, 2, fx, f64);

    fxfrac = tm_name_as(tm, tm_reserve(tm, B_NEW, FX, 0, 0), "fxfrac");
    fxfrac = tm_interv(tm, fxfrac, 2, fxfrac, i64frac);


    // GBPUSD: fx & {dom:GBP, for:USD}

    GBPUSD = tm_name_as(tm, tm_reserve(tm, B_NEW, 0, 0, 0), "GBPUSD");
    ...
    btypeid_t f64Tree;
    f64Tree = tm_name_as(tm, tm_reserve(tm, B_NEW, 0, 0, 0), "f64Tree");
    ...
    btypeid_t tree;
    tree = tm_name_as(tm, tm_reserve(tm, B_NEW, 0, 0, 0), "tree");
